...
const PowerSourceNode = ({ data }) => (
  <div style={styles.node('#22c55e', '#052e16')}>
    ...
    <Handle type="source" position={Position.Right} style={styles.handle('#22c55e')} />
  </div>
);
...
const BreakerNode = ({ data }) => (
  <div style={styles.node('#facc15', '#1c1917')}>
    <Handle type="target" position={Position.Left} style={styles.handle('#facc15')} />
    <div style={styles.nodeLabel}>🔌 {data.label}</div>
    <div style={styles.nodeSub}>Breaker</div>
    <Handle type="source" position={Position.Right} style={styles.handle('#facc15')} />
  </div>
);
...
const DeviceNode = ({ data }) => (
  <div style={styles.node('#60a5fa', '#0f172a')}>
    <Handle type="target" position={Position.Left} style={styles.handle('#60a5fa')} />
    <div style={styles.nodeLabel}>💡 {data.label}</div>
...
const initialEdges = [
  ...
  { id: 'e2-3', source: '2', target: '3', animated: true, style: { stroke: '#facc15', strokeWidth: 2 } },
  { id: 'e2-4', source: '2', target: '4', animated: true, style: { stroke: '#facc15', strokeWidth: 2 } },
];
...
export default function PowerFlowTest() {
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', background: '#0a0a0a' }}>
      ...
      <div style={statusBox}>
        <div style={{ color: '#22c55e', fontWeight: 700, marginBottom: 4 }}>SYSTEM STATUS</div>
        <div style={{ color: '#a3a3a3', fontSize: 12 }}>Nodes: {nodes.length}</div>
        <div style={{ color: '#a3a3a3', fontSize: 12 }}>Connections: {edges.length}</div>
        <div style={{ color: '#22c55e', fontSize: 12, marginTop: 4 }}>● SAFE</div>
      </div>
